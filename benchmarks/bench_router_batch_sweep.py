#!/usr/bin/env python3
"""Batch-size sweep for the per-head router KV-skip decode kernel.

Companion to bench_router_kv_skip.py. Fixes context / window / heads and sweeps the
batch size, so you can see how the router's pruning speedup behaves as the kernel
goes from GPU-underfilled (small batch) to bandwidth-bound (large batch).
...
Usage:
  python benchmarks/bench_router_batch_sweep.py
  python benchmarks/bench_router_batch_sweep.py --seq-len 8192 --window 128 \
      --batches 1 4 16 64 --mixed-fractions 0.5 0.7 0.9
"""
import argparse
...
def main():
    ...
    for bs in args.batches:
        cfg = Config(
            batch_size=bs, seq_len=args.seq_len, num_qo_heads=args.num_qo_heads,
    ...
    desc = (f"BATCH SWEEP  |  seq_len={args.seq_len}  window={args.window}  "
            f"heads={args.num_qo_heads}qo/{args.num_kv_heads}kv  head_dim={args.head_dim}  "
            f"{args.kv_dtype}  {path} decode")
    print_tables("batch", rows, desc, args.mixed_fractions)
